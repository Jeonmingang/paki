        ...
        new BukkitRunnable(){ double x=start.getX(), y=start.getY(), vx=stepX; boolean centered=false;
            @Override public void run(){ y+=stepY; x+=vx; if (x<=leftX || x>=rightX){ vx=-vx; x+=vx; } item.teleport(new Location(w,x,y,start.getZ()));
                ...
                if (y>=yTop){ this.cancel(); item.remove();
                    ...
                    if (target!=null && target.getBlock().getState() instanceof org.bukkit.block.Container){
                        org.bukkit.block.Container c=(org.bukkit.block.Container)target.getBlock().getState();
                        Inventory inv=c.getInventory(); ItemStack it=vis.clone(); it.setAmount(1);
                        java.util.HashMap<Integer, ItemStack> left=inv.addItem(it); if (!left.isEmpty()) w.dropItem(target.clone().add(0.5,0.7,0.5), it);
                        // cross-version sound fallback (no direct reference to missing constants)
                        org.bukkit.Sound s;
                        try { s = org.bukkit.Sound.valueOf("BLOCK_HOPPER_INSIDE"); }
                        catch (IllegalArgumentException ex1) {
                            try { s = org.bukkit.Sound.valueOf("BLOCK_HOPPER"); }
                            catch (IllegalArgumentException ex2) { s = org.bukkit.Sound.UI_BUTTON_CLICK; }
                        }
                        w.playSound(target, s, 0.6f, 1.1f);
                        w.spawnParticle(Particle.SPELL_INSTANT, target.clone().add(0.5,0.7,0.5), 12, 0.2,0.2,0.2, 0.05);
