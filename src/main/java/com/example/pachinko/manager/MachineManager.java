...
public class MachineManager {
    ...
    public void reloadConfigValues() {
        ...
        // stages
        stages = new ArrayList<StageDef>();
        java.util.List<java.util.Map<?,?>> list = cfg.getMapList("stages");
        for (java.util.Map<?,?> m : list) {
            StageDef s = new StageDef();
            Object idObj = m.get("id");
            if (idObj == null) continue;
            if (idObj instanceof Number) s.id = ((Number) idObj).intValue();
            else {
                try { s.id = Integer.parseInt(String.valueOf(idObj)); }
                catch (Exception ex) { continue; }
            }

            Object nameObj = m.get("name");
            s.name = (nameObj != null) ? String.valueOf(nameObj) : ("STAGE " + s.id);

            Object cupObj = m.get("cup");
            s.cup = (cupObj instanceof Number) ? ((Number) cupObj).intValue() : 50;

            Object nscObj = m.get("next-stage-chance");
            s.nextStageChance = (nscObj instanceof Number) ? ((Number) nscObj).doubleValue() : 0.2D;

            java.util.Map<?,?> lane = (java.util.Map<?,?>) m.get("lane");
            if (lane != null) {
                Object lo = lane.get("left");
                Object co = lane.get("center");
                Object ro = lane.get("right");
                s.laneLeft = (lo instanceof Number) ? ((Number) lo).doubleValue() : baseProb.left;
                s.laneCenter = (co instanceof Number) ? ((Number) co).doubleValue() : baseProb.center;
                s.laneRight = (ro instanceof Number) ? ((Number) ro).doubleValue() : baseProb.right;
            } else {
                s.laneLeft = baseProb.left; s.laneCenter = baseProb.center; s.laneRight = baseProb.right;
            }

            Object enterObj = m.get("enter-broadcast");
            s.enterBroadcast = (enterObj != null) ? String.valueOf(enterObj) : "";

            Object upObj = m.get("up-broadcast");
            s.upBroadcast = (upObj != null) ? String.valueOf(upObj) : "";

            stages.add(s);
        }
        // sort by id
        stages.sort(java.util.Comparator.comparingInt(a -> a.id));
    }
