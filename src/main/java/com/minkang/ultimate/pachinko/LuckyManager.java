...
public class LuckyManager {
    ...
    private void loadStagesFromConfig(){
        ...
        if (list!=null && !list.isEmpty()){
            ...
            try{ Object v = m.get("click-min"); if (v!=null) sc.clickMin = Integer.parseInt(String.valueOf(v)); }catch(Exception ignored){}
            try{ Object v = m.get("click-max"); if (v!=null) sc.clickMax = Integer.parseInt(String.valueOf(v)); }catch(Exception ignored){}
    ...
    public boolean handleClick(Player p, Machine m){
        ...
        StageCfg sc = current(s);
        int perClick = Math.max(1, (sc.clickMin!=null && sc.clickMax!=null) ? (Math.min(sc.clickMin, sc.clickMax) + rnd.nextInt(Math.abs(sc.clickMax - sc.clickMin)+1)) : sc.clickGain);
    ...
    public void onCenterDuringLucky(Player p, Machine m){
        ...
        int add = 0;
        if (plugin.getConfig().getBoolean("lucky.allow-dynamic-cap", false)){
            add = plugin.getConfig().getInt("lucky.cap-increase-on-center",0);
        }
        if (add>0){
            s.cap += add;
            s.remaining += add;
