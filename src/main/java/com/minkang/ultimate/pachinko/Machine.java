...
public class Machine {
    ...
    public void build(MachineRegistry reg){
        ...
        // button
        Block bbtn = getStartButtonLocation(reg).getBlock();
        bbtn.setType(Material.STONE_BUTTON, false);
        if (bbtn.getBlockData() instanceof Directional){
            Directional d = (Directional)bbtn.getBlockData();
            d.setFacing(BlockFace.SOUTH);
            bbtn.setBlockData(d, false);
        }
        // start block
