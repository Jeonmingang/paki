...
public class Main extends JavaPlugin {
    ...
    public int getJackpotSlotIndex(int cols){
        String val = getConfig().getString("slot.jackpot-slot","center");
        int idx;
        if (val==null) val="center";
        val = val.trim().toLowerCase();
        if (val.equals("left")) idx = 0;
        else if (val.equals("right")) idx = Math.max(0, cols-1);
        else if (val.equals("center")) idx = cols/2;
        else {
            try{
                int oneBased = Integer.parseInt(val);
                idx = Math.max(0, Math.min(cols-1, oneBased-1));
            }catch(Exception e){
                idx = cols/2;
            }
        }
