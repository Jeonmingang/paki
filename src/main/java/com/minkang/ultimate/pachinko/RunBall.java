...
public class RunBall {
    ...
    public void begin(){
        ...
        ball = w.spawn(spawn, ArmorStand.class, as -> {
            ...
            as.setHelmet(new org.bukkit.inventory.ItemStack(plugin.getBallMaterial()));
        });
    ...
    private void doSlotSequential(boolean isCenter){
        ...
        if (new java.util.Random().nextInt(100) >= rollChance){
            ...
            Bukkit.getScheduler().runTaskLater(plugin, ()->{
                ...
                Bukkit.getScheduler().runTaskLater(plugin, ()->{
                    ...
                    if (allEq && plugin.getConfig().getBoolean("slot.jackpot-when-all-equal", true)){
                        ...
                        Location fwLoc = machine.getPayoutHoleLocation(plugin.getRegistry());
                        org.bukkit.entity.Firework fw = fwLoc.getWorld().spawn(fwLoc, org.bukkit.entity.Firework.class);
                        org.bukkit.inventory.meta.FireworkMeta meta = fw.getFireworkMeta();
                        meta.addEffect(org.bukkit.FireworkEffect.builder().with(org.bukkit.FireworkEffect.Type.BALL).withColor(Color.AQUA).trail(true).build());
                        meta.setPower(1);
