...
public class RunBall {
    ...
    private void finish(int gain){
        if (ball!=null && !ball.isDead()) {
            ...
            ball.remove();
            if (gain>0){
                // Jackpot sound only (no fireworks/particles heavy)
                try {
                ...
                // Lucky entry?
                java.util.List<Integer> luckyCols = plugin.getConfig().getIntegerList("lucky.columns");
                if (plugin.getConfig().getBoolean("lucky.enabled", true) && luckyCols != null){
                    if (luckyCols.contains(c)){
