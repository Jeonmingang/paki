...
public class RunBall {
    ...
    private void doSlotSequential(boolean isCenter){
        ...
        Bukkit.getScheduler().runTaskLater(plugin, ()->{
            ...
            player.sendMessage(prefix + "슬롯: ["+A+"] - [ ] - [ ]");
                if (spin){ try{ player.playSound(player.getLocation(), org.bukkit.Sound.valueOf(loopSound), .7f, 1.0f);}catch(Exception ignored){} }
                Bukkit.getScheduler().runTaskLater(plugin, ()->{
                    player.sendMessage(prefix + "슬롯: ["+A+"] - ["+B+"] - [ ]");
                    if (spin){ try{ player.playSound(player.getLocation(), org.bukkit.Sound.valueOf(loopSound), .7f, 1.1f);}catch(Exception ignored){} }
                    int lastDelay = delay + (lastMax>lastMin ? rnd.nextInt(lastMax-lastMin+1)+lastMin : lastMin);
                    Bukkit.getScheduler().runTaskLater(plugin, ()->{
                        player.sendMessage(prefix + "슬롯: ["+A+"] - ["+B+"] - ["+C+"]");
    ...
    private void finish(){
        if (ball!=null && !ball.isDead()) {
            ...
            if (!plugin.getLucky().isInLucky(player)){
                ...
                // lucky state: optional forced center by config
                boolean force = plugin.getConfig().getBoolean("lucky.force-center-during-jackpot", false);
                if (force){
                    int ch = Math.max(0, Math.min(100, plugin.getConfig().getInt("lucky.force-center-chance", 0)));
                    if (rnd.nextInt(100) < ch) isCenter = true;
                }
            }
    ...
    private static void playLoopSound(Player p, String key, float vol, float pit){
        try{ p.playSound(p.getLocation(), Sound.valueOf(key), vol, pit);}catch(Exception ignored){}
    }
