...
import com.minkang.ultimate.pachinko.model.Stage;
...
public class PachinkoCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (sub.equals("리로드")) {
            ...
            try {
                plugin.getMachinesConfig().save(manager.getMachinesFile());
        ...
        if (sub.equals("설치")) {
            ...
            if (m != null) {
                sender.sendMessage(Text.prefix(plugin) + "기계 #" + id + " 설치 완료 (" + m.getFacing().name() + ")");
            } else {
                sender.sendMessage(Text.prefix(plugin) + "기계 설치 실패: 동일 ID가 이미 존재");
            }
            return true;
        }
        ...
        if (sub.equals("구슬지급")) {
            if (args.length < 3) {
                sender.sendMessage(Text.prefix(plugin) + "/파칭코 구슬지급 <닉> <개수> [<기계번호(옵션)>]");
                return true;
            }
        ...
        if (sub.equals("설정")) {
            ...
            com.minkang.ultimate.pachinko.model.Machine m = manager.getMachine(id);
            if (m == null) {
            ...
            if (args.length >= 4 && args[3].equalsIgnoreCase("손")) {
                ...
                if (ok) {
                    sender.sendMessage(Text.prefix(plugin) + "기계 #" + id + " 의 구슬 템플릿을 손에 든 아이템으로 설정했습니다.");
                } else {
                    sender.sendMessage(Text.prefix(plugin) + "손에 든 아이템을 구슬로 설정할 수 없습니다.");
                }
                return true;
            }
            if (args.length >= 4 && args[3].equalsIgnoreCase("기본")) {
                manager.resetBallTemplate(m);
                sender.sendMessage(Text.prefix(plugin) + "기계 #" + id + " 의 구슬 템플릿을 기본값으로 되돌렸습니다.");
                return true;
            }
            if (args.length >= 4 && args[3].equalsIgnoreCase("잠금")) {
                ...
                sender.sendMessage(Text.prefix(plugin) + "잠금 설정을 변경했습니다: " + (m.isLockBallToMachine() ? "켜짐" : "꺼짐"));
                return true;
            }
            sender.sendMessage(Text.prefix(plugin) + "사용법: /파칭코 설정 구슬 <기계번호> <손|기본|잠금 [켜|끄]>");
            return true;
