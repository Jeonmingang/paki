...
public class PachinkoCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (args.length == 0) {
            sender.sendMessage(Text.prefix(plugin) + "/파칭코 <설치|삭제|시작|구슬지급|목록|리로드|설정>");
            return true;
        }
        ...
        if (sub.equals("삭제")) {
            if (args.length < 2) {
                sender.sendMessage(Text.prefix(plugin) + "/파칭코 삭제 <기계번호>");
                return true;
            }
            int id;
            try { id = Integer.parseInt(args[1]); } catch (NumberFormatException e) {
            ...
            sender.sendMessage(Text.prefix(plugin) + "기계 #" + id + " 삭제 완료");
            return true;
        }
if (sub.equals("시작")) {
            if (args.length < 2) {
