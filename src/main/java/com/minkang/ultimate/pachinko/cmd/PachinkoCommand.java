...
package co
            ...
            if (sub.equals("랭킹")){
                java.io.File f = new java.io.File(com.minkang.ultimate.pachinko.Main.getInst().getDataFolder(), "ranking.yml");
                ...
                if (arr==null || arr.isEmpty()){ com.minkang.ultimate.pachinko.util.Text.msg(p,"&7기록이 없습니다."); return true; }
                // sort by payout desc then stage desc
                java.util.Collections.sort(arr, new java.util.Comparator<java.util.Map<String,Object>>(){
                    public int compare(java.util.Map<String,Object> a, java.util.Map<String,Object> b){
                        int pa=((Number)a.getOrDefault("payout",0)).intValue();
                        int pb=((Number)b.getOrDefault("payout",0)).intValue();
                        if (pa!=pb) return Integer.compare(pb,pa);
                        int sa=((Number)a.getOrDefault("stage",0)).intValue();
                        int sb=((Number)b.getOrDefault("stage",0)).intValue();
                        return Integer.compare(sb,sa);
                    }
                });
                int n=0; com.minkang.ultimate.pachinko.util.Text.msg(p,"&6[파칭코 랭킹 TOP 10]");
                for (java.util.Map<String,Object> r: arr){
                    String nm = String.valueOf(r.get("player"));
                    int st = ((Number)r.getOrDefault("stage",0)).intValue();
                    int po = ((Number)r.getOrDefault("payout",0)).intValue();
                    long tt = ((Number)r.getOrDefault("time",0)).longValue();
                    java.text.SimpleDateFormat sdf=new java.text.SimpleDateFormat("MM/dd HH:mm");
                    com.minkang.ultimate.pachinko.util.Text.msg(p, "&e"+(++n)+". &f"+nm+" &7- 스테이지 &f"+st+" &7/ 배출 &f"+po+" &7("+sdf.format(new java.util.Date(tt))+")");
                    if (n>=10) break;
                }
                return true;
            }
    m.minkang.ultimate.pachinko.cmd;
...
public class PachinkoCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] a) {
        ...
        if (a.length==0 || a[0].equalsIgnoreCase("도움말")){
            ...
            Text.msg(p, "&6/파칭코 구슬지급 [닉] [개수] &7- 파칭코 구슬 지급(기본 1개)");
            return true;
        }
        ...
        if (a[0].equalsIgnoreCase("설정")){
            ...
            if (sub.equals("스테이지테이블")){
                if (a.length<3){ Text.msg(p,"&c/파칭코 설정 스테이지테이블 (추가|삭제|확률|캡) / 확변 / 확변비율 / 시단 / 잭팟확률 "); return true; }
            ...
            if (sub.equals("확변")){
                if (a.length<4){ Text.msg(p,"&c/파칭코 설정 확변 <확률(1/n 또는 소수)> <회수>"); return true; }
                String prob = a[2];
                int spins = toInt(a[3],100);
                Main.get().getConfig().set("jpachinko.kakuhen.chance", prob);
                Main.get().getConfig().set("jpachinko.kakuhen.spins", spins);
                Main.get().saveConfig();
                Text.msg(p,"&a확변 설정 저장: chance="+prob+" spins="+spins);
                return true;
            }
            if (sub.equals("확변비율")){
                if (a.length<3){ Text.msg(p,"&c/파칭코 설정 확변비율 <0~1>"); return true; }
                double rate = toD(a[2], 0.5);
                Main.get().getConfig().set("jpachinko.kakuhen.rateOnJackpot", rate);
                Main.get().saveConfig();
                Text.msg(p,"&a확변 비율="+rate);
                return true;
            }
            if (sub.equals("시단")){
                if (a.length<4){ Text.msg(p,"&c/파칭코 설정 시단 <확률(1/n 또는 소수)> <회수>"); return true; }
                String prob = a[2];
                int spins = toInt(a[3],50);
                Main.get().getConfig().set("jpachinko.jidan.chance", prob);
                Main.get().getConfig().set("jpachinko.jidan.spins", spins);
                Main.get().saveConfig();
                Text.msg(p,"&a시단 설정 저장: chance="+prob+" spins="+spins);
                return true;
            }
            if (sub.equals("잭팟확률")){
                if (a.length<4){ Text.msg(p,"&c/파칭코 설정 잭팟확률 <스테이지index> <확률(1/n 또는 소수)>"); return true; }
                int idx = toInt(a[2],1);
                String prob = a[3];
                java.util.List<java.util.Map<?,?>> stages = Main.get().getConfig().getMapList("stages");
                if (idx<0 || idx>=stages.size()){ Text.msg(p,"&c스테이지 범위 오류"); return true; }
                java.util.Map m = (java.util.Map) stages.get(idx);
                m.put("tripleMatchChance", prob);
                stages.set(idx, m);
                Main.get().getConfig().set("stages", stages);
                Main.get().saveConfig();
                Text.msg(p,"&a스테이지 "+idx+" 잭팟확률="+prob);
                return true;
            }
