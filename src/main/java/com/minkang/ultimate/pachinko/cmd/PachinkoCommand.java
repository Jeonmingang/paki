...
public class PachinkoCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        String sub = args[0];
        if (sub.equalsIgnoreCase("도움말")) { help(sender); return true; }
        ...
        if (sub.equalsIgnoreCase("목록")) {
            ...
            boolean first = true;
            for (Integer id : mm.getMachines().keySet()) { if (!first) sb.append(", "); sb.append("#").append(id); first = false; }
        ...
        if (sub.equalsIgnoreCase("랭킹")) {
            ...
            int i=1; for (java.util.Map.Entry<String,Integer> e : top) { Text.msg(sender, "&e"+(i++)+". &f"+e.getKey()+" &7- &b"+e.getValue()+"점"); }
    ...
    private void help(CommandSender s) {
        ...
        Text.msg(s, "&b/파칭코 테스트 <번호> &7- 공 시뮬레이션 실행 (오피)");
        Text.msg(s, "&b/파칭코 랭킹 &7- TOP10 출력");
    }
