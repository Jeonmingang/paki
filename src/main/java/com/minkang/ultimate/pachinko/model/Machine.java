...
public class Machine {
    ...
    public Machine(Main plugin, int id, World world, Location base, BlockFace facing) {
        ...
        this.id = id;
        this.world = world;
        this.base = base.getBlock().getLocation(); // normalize
        this.facing = facing;
    }
    ...
    public void buildStructure() {
        ...
        // Floors: y=1..6 (glass,bar alternation)
        for (int y = 1; y <= 6; y++) {
            Material mat = (y % 2 == 1) ? Material.GLASS : Material.IRON_BARS;
            for (int x = -4; x <= 3; x++) setBlock(relative(x, y, 0), mat);
        }
        // Top hoppers at y=7 (8 wide)
        for (int x = -4; x <= 3; x++) setBlock(relative(x, 7, 0), Material.HOPPER);
    }

    public void removeStructure() {
        for (int x = -4; x <= 3; x++) setBlock(relative(x, 7, 0), Material.AIR);
        for (int y = 1; y <= 6; y++) for (int x = -4; x <= 3; x++) setBlock(relative(x, y, 0), Material.AIR);
    ...
    public void onCoalClicked(Player p) {
        ...
        if (drawTickets >= max) {
            ...
            drawTickets = 0; // 자동 연속 추첨
            runEntryDrawsSequential(p, attempts);
        } else {
            drawTickets -= 1;
            doThreeDigitDraw(true, p,
                new Runnable(){ public void run(){ // 성공 → IN_STAGE
                ...
                new Runnable(){ public void run(){ // 실패 → 남은 추첨권으로 다시 시도 가능
    ...
    private void runEntryDrawsSequential(final Player actor, final int attemptsRemaining) {
        ...
        doThreeDigitDraw(true, actor,
            ...
            new Runnable(){ public void run(){ // 실패 → 남은 횟수 연속 시도
    ...
    private void raiseBallVisual() {
        ...
        final int destX = (int) Math.floor(Math.random() * 8) - 4; // -4..3
        final boolean isCenter = (destX == 0);

        final Location start = locGold.clone().add(0.5, 1.0, 0.5);
        final Location end = relative(destX, 7, 0).add(0.5, 1.0, 0.5);
            ...
            public void run() {
                ...
                if (i >= steps) {
                    ...
                    if (lastActor != null) {
                        ...
                        if (op != null && op.isOnline()) ((Player)op).sendMessage(Text.prefix(plugin) + msg);
                    }
                    cancel();
                }
                i++;
