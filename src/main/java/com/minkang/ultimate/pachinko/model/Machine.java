...
public class Machine {
    ...
    public void onGoldClicked(Player p) {
        if (state == State.PRE_ENTRY) {
            ...
            if (drawTickets >= max) { p.sendMessage(Text.prefix(plugin) + "추첨권이 이미 최대치입니다."); return; }
            drawTickets++;
    ...
    public void onCoalClicked(Player p) {
        ...
        int max = plugin.getConfig().getInt("chance.maxTickets", 5);
        if (drawTickets <= 0) { p.sendMessage(Text.prefix(plugin) + "추첨권이 없습니다."); return; }
    ...
    private void endStage(boolean jackpot) {
        ...
        if (msg != null && !msg.isEmpty()) Bukkit.broadcastMessage(Text.prefix(plugin) + msg);
    ...
    // 금블럭: 애니메이션 후 중앙이면 콜백 실행 (IN_STAGE 전용)
    private void raiseBallAndThen(final Player owner, final boolean requireCenter, final Runnable onCenter) {
        if (animating) return;
        animating = true;
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (i >= steps) {
                    ...
                    animating = false;
                    String msg = plugin.getConfig().getString(isCenter ? "messages.centerHit" : "messages.centerMiss", isCenter ? "중앙!" : "꽝!");
                    if (owner != null) owner.sendMessage(Text.prefix(plugin) + msg);
                    if (!requireCenter || isCenter) { if (onCenter != null) onCenter.run(); }
                    cancel();
                }
                i++;
            }
