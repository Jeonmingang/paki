...
public class Machine {
    ...
    public Machine(Main plugin, int id, String worldName, Location base, int slots){
        ...
        // initialize stageCup from config
    ...
    public void endStage(Player actor, String reason){
        // stop BGM
        BgmController.stopAll();
        Text.msg(actor, "&c스테이지 종료 &7- "+reason);
    ...
    public void payOut(Player actor){
        ...
        ItemStack ball = ItemUtil.defaultBall(plugin);
        try {
    ...
    private void animateOneStep(int fromSlot, int toSlot, Runnable onFinish){
        ...
        final ItemStack ball = ItemUtil.defaultBall(plugin);
        final Item item = start.getWorld().dropItem(start, ball);
        item.setPickupDelay(Integer.MAX_VALUE);
        item.setGravity(false);
        final int totalTicks = 12; // 0.6s
        new BukkitRunnable(){
            int t = 0;
            @Override public void run(){
                t++;
                double tt = Easing.clamp01(t/(double)totalTicks);
                double smooth = Easing.easeInOutQuad(tt);
