...
public class Machine {
    ...
    private final Location base; // bottom center (gold block above?) ground level
    private ItemStack ballItem;
    private int[] weights = new int[]{14,14,14,6,14,14,14}; // default
    private List<Location> hopperLocations = new ArrayList<>(7);
    private Location goldButton; // gold block for payout (right-click)
    private int stageIndex = 0; // 0..stages-1
    private int pendingSpins = 0; // 0..config max
    private int pendingPayout = 0; // balls to give on gold click
    private int currentPayout = 0; // stage running payout so far
    ...
    public Inventory getHopperInv(int slotIndexOneBased) {
