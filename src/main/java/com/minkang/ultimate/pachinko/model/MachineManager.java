...
public class MachineManager {
    ...
    public Machine installMachine(Player p, int id) {
        ...
        for (int row=1; row<=6; row++) for (int col=-3; col<=3; col++) w.getBlockAt(x0+col, y0+row, z0).setType((row%2==0)?Material.IRON_BARS:Material.GLASS);
        java.util.List<Location> hopperLocs = new java.util.ArrayList<>();
        for (int i=-3;i<=3;i++) { Block slab = w.getBlockAt(x0+i, y0+7, z0); slab.setType(Material.SMOOTH_STONE_SLAB); Block hop = w.getBlockAt(x0+i, y0+8, z0); hop.setType(Material.HOPPER); hopperLocs.add(hop.getLocation()); }
        ...
        machines.put(id, m);
        saveMachines();
        Text.msg(p, "&a파칭코 기계 #" + id + " 설치 완료! 중앙 금블럭: 우클릭으로 보상 수령/플레이.");
        return m;
    }
    ...
    public boolean deleteMachine(Player p, int id) {
        ...
        for (int y=0;y<10;y++) for (int x=-4;x<=4;x++) { Block b = w.getBlockAt(x0+x, y0+y, z0); if (b.getType()!=Material.AIR) b.setType(Material.AIR); }
    ...
    private void showHud(Player p, Machine m) {
        if (p == null) return;
        String msg = Text.color("&b" + stageName(m) + " &7| &f지급:&e" + m.getCurrentPayout() + "&7/&e" + stageCap(m) + " &7| &f추첨:&e" + m.getPendingSpins());
        try { p.spigot().sendMessage(ChatMessageType.ACTION_BAR, new TextComponent(msg)); } catch (Throwable t) { }
    }
    private int addPayoutWithCap(Player p, Machine m, int amount) {
        int cap = stageCap(m);
