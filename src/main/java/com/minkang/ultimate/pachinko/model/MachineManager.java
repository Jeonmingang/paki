...
public class MachineManager {
    ...
    public void playStageBgm(Player p, int stageIndex){
        ...
        Object __bgm = stages.get(stageIndex).get("bgm");
        String snd = (__bgm == null) ? "BLOCK_NOTE_BLOCK_BIT" : String.valueOf(__bgm);
        try {
    ...
    public void startFever(org.bukkit.entity.Player p, Machine m){
        ...
        if (plugin.getConfig().getBoolean("broadcast.enabled", true)){
            org.bukkit.Bukkit.broadcastMessage(com.minkang.ultimate.pachinko.util.Text.color("&6[파칭코] &f#"+m.getId()+" &cFEVER TIME 시작!"));
        }
    }
}
}
