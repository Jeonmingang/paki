...
public int chooseSlotWeighted(Machine m){
    ...
    public void enterStage(Machine m, org.bukkit.entity.Player p){
        ...
        m.setStageIndex(1);
        idx = 1; idx = 1;  int cup = ((Number)stages.get(0).getOrDefault("cup", 200)).intValue();
        m.setStageCup(cup);
        idx = Math.max(1, m.getStageIndex()); if (idx > stages.size()) idx = stages.size();
                        idx = Math.max(1, m.getStageIndex()); if (idx > stages.size()) idx = stages.size();
        // + ranking record
                        try{
                            String stageName = String.valueOf(stages.get(Math.min(Math.max(1, m.getStageIndex())-1, stages.size()-1)).getOrDefault("name","Stage #"+m.getStageIndex()));
    ...
    private void scheduleAutoAdvance(Machine m){
        ...
        int taskId = org.bukkit.Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable(){
            public void run(){
                ...
                java.util.List<java.util.Map<String,Object>> stages = (java.util.List<java.util.Map<String,Object>>)(java.util.List<?>) plugin.getConfig().getMapList("stages");
                int idx = Math.max(1, m.getStageIndex()); if (idx > stages.size()) idx = stages.size();
        String ac = String.valueOf(stages.get(Math.max(1, m.getStageIndex())-1).getOrDefault("advanceChance", "0"));
                double prob = chanceToDouble(ac);
                if (prob > 0){
                    if (Math.random() < prob){
                        // advance stage
                        m.setStageIndex(idx+1);
                        int cup = ((Number)stages.get(Math.min(Math.max(1, m.getStageIndex())-1, stages.size()-1)).getOrDefault("cup", m.getStageCup()+50)).intValue();
                        m.setStageCup(cup);
                        idx = Math.max(1, m.getStageIndex()); if (idx > stages.size()) idx = stages.size();
        // + ranking record
                        try{
                        ...
                        String bgm = String.valueOf(stages.get(Math.min(Math.max(1, m.getStageIndex())-1, stages.size()-1)).getOrDefault("bgm","MUSIC_DISC_STRAD"));
