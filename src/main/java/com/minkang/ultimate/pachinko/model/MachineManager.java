...
public class MachineManager {
    ...
    public void giveBall(org.bukkit.entity.Player p, int amount){
        ...
        if (ball==null) ball = ItemUtil.readItem(plugin.getConfig().getConfigurationSection("defaultBall"));
        if (ball==null) return;
        ball = ball.clone();
...
public int chooseSlotWeighted(Machine m){
    ...
    public void startST(org.bukkit.entity.Player p, Machine m){
        ...
        if (plugin.getConfig().getBoolean("broadcast.enabled", true)){
            org.bukkit.Bukkit.broadcastMessage(com.minkang.ultimate.pachinko.util.Text.color("&6[파칭코] &f#"+m.getId()+" &c확변(ST) 돌입!"));
        }
    }
    ...
    public void startJidan(org.bukkit.entity.Player p, Machine m){
        ...
        if (plugin.getConfig().getBoolean("broadcast.enabled", true)){
            org.bukkit.Bukkit.broadcastMessage(com.minkang.ultimate.pachinko.util.Text.color("&6[파칭코] &f#"+m.getId()+" &b시단 시작!"));
        }
    }
    ...
    public void endSpecialModes(org.bukkit.entity.Player p, Machine m){
        ...
        if (plugin.getConfig().getBoolean("broadcast.enabled", true)){
            org.bukkit.Bukkit.broadcastMessage(com.minkang.ultimate.pachinko.util.Text.color("&6[파칭코] &f#"+m.getId()+" &7특수모드 종료."));
        }
    }
